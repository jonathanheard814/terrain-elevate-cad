...
def main() -> None:
    ...
    actuator_effort_n = 2 * 3.141592653589793 * actuator["screw_efficiency_ASSUMED"] * actuator["motor_nominal_torque_Nm_SRC"] * actuator["gear_ratio_ASSUMED"] * actuator["gearbox_efficiency_ASSUMED"] / screw_lead_m
    actuator_velocity_mps = actuator["motor_nominal_speed_rpm_SRC"] / actuator["gear_ratio_ASSUMED"] * screw_lead_m / 60
...
physicsBasisPath = fullfile(pwd, "data", "te_v059_physics_elements.json");
...
physicsBasis = jsondecode(fileread(physicsBasisPath));
...
terrainElevateParams.cornerActuatorVelocity_mps = {actuator_velocity_mps:.6f};
terrainElevateParams.cornerActuatorEffort_N = {actuator_effort_n:.3f};
terrainElevateParams.wheelDriveEffort_Nm = 60.0;
terrainElevateParams.screwLead_m = {screw_lead_m:.6f};
terrainElevateParams.screwEfficiency = {actuator["screw_efficiency_ASSUMED"]:.6f};
terrainElevateParams.localControlLoop_s = {combined["control_loop_period_ms_ASSUMED"] / 1000:.6f};
terrainElevateParams.sensorProcessPeriod_s = {combined["sensor_process_period_ms_ASSUMED"] / 1000:.6f};
...
disp("Loaded physics basis:");
disp(physicsBasis.sensor_and_control_timing);
"""
    ...
    simscape_notes = {
        ...
        "physics_basis": "../data/te_v059_physics_elements.json",
        ...
        "contact_and_control_basis": "Use te_v059_physics_elements.json for tire/stair contact, actuator motor-screw limits, sensor timing, and local control-loop timing.",
        "warning": "URDF contains simplified collision/visual meshes and explicit joints for dynamics setup; the detailed STEP is the richer CAD reference for SolidWorks.",
    }
